...
// SIMPLE BAD WORD LIST (edit anytime)
const BAD_WORDS = [
  "fuck",
  "shit",
  "bitch",
...
client.on(Events.MessageCreate, async message => {
  try {
    ...
    if (LINK_REGEX.test(message.content)) {
      ...
      warnUser(message, userId, "Sending links is not allowed");
      return;
    }
    ...
    // keep last 5 seconds messages only
    ...
    if (filtered.length > 5) {
      await message.delete().catch(() => {});
      warnUser(message, userId, "Spamming detected");
    }
...
async function warnUser(message, userId, reason) {
  ...
  let warns = userWarnings.get(userId);
  warns++;
  ...
  if (warns >= 3) {
    try {
      await member.timeout(10 * 60 * 1000, "Reached 3 warnings");
      userWarnings.set(userId, 0);

      message.channel.send(
        `🔨 <@${userId}> has been timed out for 10 minutes`
      );
    } catch (err) {
